  ...
  <BlogLayout :categories="categories">
    ...
    <section class="mb-12">
      <template v-if="paginated_posts.data.length">
        <h2 class="">Results for: <b class="text-primary font-bold">{{ author.name }}</b></h2>
      ...
      <template v-else>
        <p class="flex items-center justify-center rounded-md text-center font-bold bg-base-200 py-12">No posts found for author {{ author.name }}</p>
      </template>
    </section>
  </BlogLayout>
</template>