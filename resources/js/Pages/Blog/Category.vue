  ...
  <BlogLayout :categories="categories">
    ...
    <section class="mb-12">
      <template v-if="paginated_posts.data.length">
        <h2 class="">Results for: <b class="text-primary font-bold">{{ category.name }}</b></h2>
        <div class="grid lg:grid-cols-2 mb-12 gap-8 mt-8">
          <ArticleCard
      ...
      <template v-else>
        <p class="flex items-center justify-center rounded-md text-center font-bold bg-base-200 py-12">No posts found for category {{ category.name }}</p>
      </template>
    </section>
  </BlogLayout>
</template>